...
カラーパレットは色番号(0～255)とそれに対応した色(0～100%)を定義します。ピクセルの描画を行う際は色番号を指定して出力を行います。以下にカラーパレットの定義例を示します。
...
| #1;2;100;0;0 | カラーパレットの定義(#1,赤) |  #1(色番号);2(RGB指定);(red;green;blue) 色は[0～100]で指定する|
...
| $ | 描画位置を行頭に戻す |  |
| - | 次の行(6pixel下)へ移動 | `$-`は次の行(6pixel下)の先頭へ移動 |
...
1. **reductionColor**: 画像の色を200色程度に減色します
